...
import { CreateEnvironmentData, Environment } from '../../../../types/environments'
...
): Promise<ApiResponse<Environment, CreateEnvironmentErrorCode>> {
  const { client, project } = args
  const path = `/v1/projects/${project}/environments`

  return await client.sendApiRequest<Environment, CreateEnvironmentErrorCode>({
    method: 'POST',
    path,
    data: args.data,
  })
